...
```
---
### COVID-19 Cases and Deaths - How did they evolve with time?

```{r states-deaths-cases, echo = FALSE, include = FALSE}
...
```{r daily-total-cases-by-state, echo = FALSE, fig.width = 14, fig.height = 6, fig.alt = "Map of US showing evolution of cases by state over time."}
case_graph = plot_geo(state_deaths_cases,
              ...
              layout(geo = list(scope = 'usa'),
                     title = 'COVID-19 Daily Cases in the US by State')
...
---
### COVID-19 Cases and Deaths - How did they evolve with time?
```{r daily-total-deaths-by-state, echo = FALSE, fig.width = 14, fig.height = 6, fig.alt = "Map of US showing evolution of deaths by state over time."}
death_graph = plot_geo(state_deaths_cases,
                      ...
                      colorscale = 'electric') %>%
              layout(geo = list(scope = 'usa'),
                     title = 'COVID-19 Daily Deaths in the US by State')
...
```{r dual-vaccine-deaths-plot, echo = FALSE, fig.width = 14, fig.height = 7, fig.alt = "Dual scatterplot of total number of US Population fully vaccinated versus time and daily number of COVID deaths versus time which shows the stages of both deaths and vaccinations in conjunction."}
coeff <- 1

merged_US_data %>%
  ggplot(aes(x = Date)) +
  geom_point(aes(y = total_new_deaths / 10), color = "red") + 
  geom_point(aes(y = Series_Complete_Yes / 10^6), color = "blue") +
  scale_color_manual(name = "Data", values = c("total_new_deaths" = "red", "Series_Complete_Yes" = "blue")) +
  scale_y_continuous(
    name = "Daily # of COVID Deaths (Tens)",
    sec.axis = sec_axis(~.*coeff, 
                        name = "Total # of People Fully Vaccinated (Millions)",
                        scales::pretty_breaks(n = 10)),
    breaks = scales::pretty_breaks(n = 10)
  ) +
  labs(title = "Total Fully Vaccinated and Daily New Deaths vs. Time",
       x = "Date",
       color = "Color") +
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
...
```{r edit-cases-data-by-season, echo = FALSE, include = FALSE}
...
seasons_deaths_cases %>%
  ggplot(aes(x = total_new_cases / 10^3, 
             ...
             fill = season)) +
  geom_boxplot() +
  labs(title = "Box Plot Of Total Daily New Cases By Season",
...
tidy(deaths_cases_main_fit)
```
Therefore:

Fall : $\hat{deaths} = 63 + 0.011 * cases$ 

Spring : $\hat{deaths} = 421 + 0.011 * cases$ 

Summer : $\hat{deaths} = 94 + 0.011 * cases$ 

Winter : $\hat{deaths} = 611 + 0.011 * cases$ 
---
### Tying Seasonality into the Relationship Between Cases and Deaths
```{r deaths-cases-int-fit}
tidy(deaths_cases_int_fit)
```
Therefore:

Fall : $\hat{d} = 302 + 0.0087 * cases$

Spring : $\hat{d} = 621 + 0.0049 * cases$

Summer : $\hat{d} = 388 + 0.00595 * cases$

Winter : $\hat{d} = 304 + 0.01687 * cases$
---
### Tying Seasonality into the Relationship Between Cases and Deaths
...
- There are some limitations to our analysis:
  - Testing capacity was not fully operational at the beginning of the pandemic
  - We are viewing things on national scale so we are missing out on local trends
  - We did not take into account the differing vaccination rates by region or
  how vaccination accessibility might not be as high in certain regions
---
<style>
.center2 {
  margin: 0;
...
---
### COVID-19 Cases and Deaths - What is the correlation? (CCF Reference Table)
...
```
---
### Exploring the Relationship Between Vaccinations and Cases/Deaths (CCF Reference Table)
...
---
### Exploring the Seasonality of COVID-19
Summary statistics for season data:
```{r edit-cases-data-by-season2, echo = FALSE}
seasons_deaths_cases <- new_US_deaths_cases %>%
  mutate(season = case_when(
  month(submission_date) %in% c(9, 10, 11) ~ "Fall",
  month(submission_date) %in% c(12, 1, 2) ~ "Winter",
